...
describe("Forgium implementation observation loop", () => {
  it("keeps Work doing for a pending Spec Flow review and only advances when ticket status is complete", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "forgium-implementation-e2e-"));
    ...
    const pending = await cli(root, ["implement", work.id, "--json"], { ...process.env, FORGIUM_PI_COMMAND: pendingPi });
    ...
    const complete = await cli(root, ["implement", work.id, "--json"], { ...process.env, FORGIUM_PI_COMMAND: completePi });
    ...
    const review = await cli(root, ["review", work.id, "--json"]);
